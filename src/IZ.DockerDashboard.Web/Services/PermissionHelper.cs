using IZ.DockerDashboard.Application.Common.Security;
...
using IZ.DockerDashboard.Application.Common.ExceptionHandlers;
...
public class PermissionHelper
{
    ...
    private async Task<List<Claim>> GetInheritedClaims(string userId)
    {
        ...
        return await _fusionCache.GetOrSetAsync(key, async _ =>
        {
            ...
            {
                var assigendRoles = await _roleManager.Roles.Where(x => roles.Contains(x.Name)).ToListAsync();
